...
test('question bank: unique IDs, four distinct choices, valid answers and explanations',()=>{assert.equal(questions.length,40);assert.equal(new Set(questions.map(q=>q.id)).size,40);for(const c of categories)assert.equal(questions.filter(q=>q.category===c).length,10);for(const q of questions){assert.equal(q.options.length,4);assert.equal(new Set(q.options).size,4);assert.ok(q.answer>=0&&q.answer<4);assert.ok(q.explanation.length>20);}});
