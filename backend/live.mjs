...
export function createLiveApi({db,config,now,one,send,limit,requireSession,session,ip,keyed}){
 ...
 async function snapshot(client,room,player,host){const players=(await client.query('SELECT id,name,team,points FROM live_players WHERE room_id=$1 ORDER BY points DESC,joined ASC',[room.id])).rows;
  ...
  return {id:room.id,pin:room.pin,category:room.category,teamMode:room.team_mode,status:room.status,roundIndex:room.round_index,total:room.questions.length,questionMs:room.question_ms,deadline:room.deadline,serverNow:now(),question:q?publicQuestion(q):null,reveal:reveal&&q?{correctIndex:q.answer,explanation:q.explanation}:null,players,teams,answeredCount:answers.length,myAnswer:player?answers.find(a=>a.player_id===player.id)?.selected??null:null,playerId:player?.id||null,isHost:host,joinUrl,qr:host&&room.status==='lobby'?await QRCode.toDataURL(joinUrl,{margin:1,width:240,errorCorrectionLevel:'M'}):null};
 }
 return async function handle(req,res,route,method,body){
  if(route==='/api/live/rooms'&&method==='POST'){
   ...
   requireThat(['All',...categories].includes(body.category)&&[5,10].includes(body.count)&&[20,30,45].includes(body.seconds)&&typeof body.teamMode==='boolean',400,'Choose a valid live game setup.');
   const bank=buildQuiz(questions,body.category,body.count),id=randomUUID(),t=now();let room;
   for(let attempt=0;attempt<8;attempt++)try{const pin=String(randomInt(100000,1000000));room=await one('INSERT INTO live_rooms(id,pin,host_id,category,questions,team_mode,question_ms,created,expires) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',[id,pin,session.user_id,body.category,JSON.stringify(bank),body.teamMode,body.seconds*1000,t,t+3*60*minute]);break;}catch(e){if(e.code!=='23505')throw e;}
  ...
  if(pinMatch&&method==='GET'){await limit('live-pin:'+keyed(ip||'unknown'),80,minute);const room=await one('SELECT id,pin,category,team_mode,status,expires FROM live_rooms WHERE pin=$1',[pinMatch[1]]);requireThat(room&&Number(room.expires)>now()&&room.status==='lobby',404,'No open lobby has that PIN.');return send(res,200,{id:room.id,pin:room.pin,category:room.category,teamMode:room.team_mode});}
